...
(function () {
  ...
  angular.module('ImageCropper', [])
      .directive('imageCrop', function () {
        return {
          templateUrl: 'templates/ng-image-crop-template.html',
          replace: true,
          restrict: 'AE',
          scope: {
            crop: '=',
          ...
          link: function (scope, element, attributes) {
            ...
            var cropHeight, cropTopMargin;
            var topMargin = 0;  // 设置图片的垂直偏移
            var cropXPos = 0, cropYPos = 0;
            var remainSpace = 93;  // todo
            // ---------- INLINE STYLES ----------- //
            scope.moduleStyles = {
            ...
            $img.onload = function () {
              ...
              if ($img.width < scope.width) {
                scope.canvasWidth = $img.width;
              }
              scope.canvasHeight = scope.canvasWidth * $img.height / $img.width;
              var tempHeight = scope.width * $img.height / $img.width;
              if (scope.height > (tempHeight + remainSpace)) {
                topMargin = (scope.height - scope.canvasHeight - remainSpace) * 0.5;
              }

              cropHeight = scope.width - 80;
              if (scope.width > scope.height) {
                cropHeight = scope.height - 80;
              }
              cropTopMargin = (scope.height - remainSpace - cropHeight) * 0.5;
              ...
              maxZoomedOutLevel = imgWidth / scope.width;
              if (maxZoomedOutLevel < 1) {
                zoom = maxZoomedOutLevel;  // todo
              }
              ...
              currentY = -topMargin * maxZoomedOutLevel / zoom;
            ...
            function moveImage(x, y) {
              function updateCurrentXY(direction) {
                switch (direction) {
                  case 1:
                  case -1:
                    currentY = currentY + y;
              ...
              updateCurrentXY(moveDirection);

              var ratio = getPixelRatio(ctx);
              var oldWidth = scope.width;
              var oldHeight = scope.height;
              ...
              ctx.clearRect(0, 0, $canvas.width, $canvas.height);
              ctx.drawImage($img, currentX, currentY , scope.width * maxZoomedOutLevel / zoom, scope.height * maxZoomedOutLevel / zoom, 0, 0, scope.width, scope.height);

              ctx.save();
              ctx.fillStyle = 'rgba(0, 0, 0, 0.65)';
              ctx.fillRect(0, 0, scope.width, scope.height);
              var restoreX = (currentX * zoom) / maxZoomedOutLevel;
              var restoreY = (currentY * zoom) / maxZoomedOutLevel;

              cropXPos = (restoreX + (parseFloat(scope.width) - parseFloat(cropHeight)) * 0.5) * maxZoomedOutLevel / zoom;
              cropYPos = (restoreY + cropTopMargin) * maxZoomedOutLevel / zoom;
              ctx.drawImage($img, cropXPos, cropYPos, cropHeight * maxZoomedOutLevel / zoom, cropHeight * maxZoomedOutLevel / zoom, (parseFloat(scope.width) - parseFloat(cropHeight)) * 0.5, cropTopMargin, cropHeight, cropHeight);
              //
              //ctx.fillRect(0, cropHeight + cropTopMargin, scope.width, cropTopMargin);
              //ctx.fillRect(0, cropTopMargin, Math.ceil((scope.width - cropHeight) * 0.5), Math.ceil(scope.height - remainSpace - 2 * cropTopMargin));
              //ctx.fillRect(Math.floor(scope.width - (scope.width - cropHeight) * 0.5), cropTopMargin, Math.ceil((scope.width - cropHeight) * 0.5), Math.ceil(scope.height - remainSpace - 2 * cropTopMargin));
            ...
            function updateDragBounds() {
              ...
              minXPos = $canvas.width - ($canvas.width * zoom);
              minYPos = $canvas.height - ($canvas.height * zoom);
            }
            ...
            function zoomImage(val) {
              ...
              var maxZoom = 1;
              if (maxZoomedOutLevel > 1) {
                maxZoom = maxZoomedOutLevel;
              }
              if (proposedZoomLevel > maxZoom || proposedZoomLevel <= 0.5) {
                return;
              }

              newWidth = $canvas.width * zoom;
              newHeight = $canvas.height * zoom;

              var tempZoomWidth = scope.width * maxZoomedOutLevel / proposedZoomLevel;
              var tempZoomHeight = scope.height * maxZoomedOutLevel / proposedZoomLevel;

              var restoreX = currentX * zoom / maxZoomedOutLevel;
              var restoreY = currentY * zoom / maxZoomedOutLevel;

              var tempHeight =  scope.width * $img.height / $img.width;
              var newXPos = (((proposedZoomLevel - zoom) * scope.width) / 2 + restoreX) * maxZoomedOutLevel / proposedZoomLevel;
              var newYPos = (((proposedZoomLevel - zoom) * tempHeight) / 2 + restoreY) * maxZoomedOutLevel / proposedZoomLevel;
              ...
              ctx.clearRect(0, 0, $canvas.width, $canvas.height);
              ctx.drawImage($img, newXPos, newYPos, tempZoomWidth, tempZoomHeight, 0, 0, scope.width, scope.height);
              ctx.save();
              ctx.fillStyle = 'rgba(0, 0, 0, 0.65)';
              ctx.fillRect(0, 0, scope.width, scope.height);

              restoreX = (newXPos * proposedZoomLevel) / maxZoomedOutLevel;
              restoreY = (newYPos * proposedZoomLevel) / maxZoomedOutLevel;

              cropXPos = (restoreX + (parseFloat(scope.width) - parseFloat(cropHeight)) * 0.5) * maxZoomedOutLevel / proposedZoomLevel;
              cropYPos = (restoreY + cropTopMargin) * maxZoomedOutLevel / proposedZoomLevel;
              ctx.drawImage($img, cropXPos, cropYPos, cropHeight * maxZoomedOutLevel / proposedZoomLevel, cropHeight * maxZoomedOutLevel / proposedZoomLevel, (parseFloat(scope.width) - parseFloat(cropHeight)) * 0.5, cropTopMargin, cropHeight, cropHeight);
              ...
              //tempCanvasContext.drawImage($img, newXPos, newYPos, cropHeight * maxZoomedOutLevel / proposedZoomLevel, cropHeight * maxZoomedOutLevel / proposedZoomLevel, 0, 0, cropHeight, cropHeight);
              //
              //var dataUrl = tempCanvas.toDataURL();
              //console.log(dataUrl);  // todo
            ...
            scope.onCanvasMouseDown = function (e) {
              ...
              //alert('start x, y: (' + startX+ ', ' + startY + ')');  // todo
              zooming = false;
              dragging = true;
            ...
            scope.onCanvasMouseMove = function (e) {
              ...
              startX = e.type === 'touchmove' ? e.changedTouches[0].clientX : e.clientX;
              startY = e.type === 'touchmove' ? e.changedTouches[0].clientY : e.clientY;
              ...
              //moveImage(currentX - diffX, currentY - diffY);  // original
            };
