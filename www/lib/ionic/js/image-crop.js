        ...
        return {
          ...
          link: function (scope, element, attributes) {
            ...
            $img.onload = function () {
              ...
              ctx.drawImage($img, 0, 0, $img.width * ratio, $img.height * ratio, 0, 0, $canvas.width, $canvas.height);
              ...
              // step 2 - reduce image to half again
              ...
              var steps = Math.ceil(Math.log($img.naturalWidth / scope.canvasWidth) / Math.log(2));
              return;
              ...
              console.log('maxZoomedInLevel', maxZoomedInLevel);  // todo
            ...
            function moveImage(x, y) {
              ...
              var newImgWidth = scope.width;
              var newImgHeight = scope.width * $img.height / $img.width;  // todo
              var ratio = getPixelRatio(ctx);  // todo
              alert('ratio: ' + ratio);  // todo
              ...
              //ctx.drawImage($img, 0, 0, $img.width * ratio, $img.height * ratio, 0, 0, scope.canvasWidth * ratio, scope.canvasHeight * ratio);
              //ctx.drawImage($img, 0, 0, $canvas.width * ratio, $canvas.height * ratio);
              //ctx.drawImage($img, 0, 0,$img.width, $img.height, 0, 0, newImgWidth, newImgHeight);
              //ctx.drawImage($img, 0, 0, scope.width, scope.width * $img.height / $img.width);
            ...
            function zoomImage(val) {
              console.log('now zooming image!');  // todo
              console.log('val', val);  // todo
              if (!val) {
                return;
              }
              ...
              console.log('newXPos', newXPos);  // todo
              console.log('newYPos', newYPos);  // todo
              console.log('newWidth', newWidth);  // todo
              console.log('newHeight', newHeight);  // todo
              // check if image is still going to fit the bounds of the box
              ctx.clearRect(0, 0, $canvas.width, $canvas.height);
              ctx.drawImage($img, newXPos, newYPos, newWidth, newHeight);
              ctx.save();
              ctx.fillStyle = 'rgba(0, 0, 0, 0.65)';
              ctx.fillRect(newXPos, newYPos, newWidth, newHeight);

              ctx.restore();
              ctx.drawImage($img, padding / (2 * proposedZoomLevel), padding / (2 * proposedZoomLevel), scope.width / proposedZoomLevel, scope.height / proposedZoomLevel, padding / 2, padding / 2, scope.width, scope.height);  // todo
              ctx.beginPath();
